...
def setup_user_debug_sssd(client: Client, provider: GenericProvider, user: str = "user1", password: str = "Secret123"):
...
def test_sssctl__analyze_list(client: Client, ldap: GenericProvider):
    ...
    :customerscenario: True
    """
    setup_user_debug_sssd(client, ldap)
...
def test_sssctl__analyze_non_default_log_location(client: Client, provider: GenericProvider):
    ...
    :customerscenario: True
    """
    user, pw = setup_user_debug_sssd(client, provider)
    client.tools.id(f"{user}@{provider.domain}")
...
def test_sssctl__analyze_pam_logs(client: Client, provider: GenericProvider):
    ...
    :customerscenario: True
    """
    setup_user_debug_sssd(client, provider)
